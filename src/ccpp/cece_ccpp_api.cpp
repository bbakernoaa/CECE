...
void cece_ccpp_sync_import_to_dev(void* data_ptr, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr) {
        std::cerr << "ERROR in cece_ccpp_sync_import_to_dev: null data_ptr" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        std::cerr << "ERROR in cece_ccpp_sync_import_to_dev: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_ccpp_sync_import_to_dev: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}
