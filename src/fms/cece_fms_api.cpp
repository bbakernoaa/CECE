/**
 * @file cece_fms_api.cpp
 * @brief Implementation of the FMS C-linkage API for CECE.
 *
 * Provides per-scheme initialization, execution, finalization, and field
 * marshalling functions that Fortran FMS driver schemes call via
 * iso_c_binding. These operate on the existing CeceInternalData structure
 * but provide per-scheme granularity needed by the FMS framework.
 *
 * Requirements: 3.1, 3.2, 3.5, 9.1-9.6, 14.1, 14.2, 14.3
 */

#include "cece/cece_fms_api.h"
...
void cece_fms_core_init(void** data_ptr, const char* config_path, int config_path_len, int nx, int ny, int nz, int* rc) {
    if (rc != nullptr) {
        *rc = 0;
    }

    std::cout << "INFO: CECE FMS core_init — starting initialization" << std::endl;
    ...
    try {
        ...
        std::cerr << "ERROR in cece_fms_core_init: Failed to parse config '" << cfg_path << "': " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
        ...
        std::cerr << "ERROR in cece_fms_core_init: Unknown error parsing config '" << cfg_path << "'" << std::endl;
        if (rc != nullptr) *rc = -1;
        if (kokkos_initialized_here && Kokkos::is_initialized()) {
            Kokkos::finalize();
        }
    ...
    std::cout << "INFO: CECE FMS core_init completed (nx=" << nx << ", ny=" << ny << ", nz=" << nz << ")" << std::endl;
}

void cece_fms_scheme_init(void* data_ptr, const char* scheme_name, int scheme_name_len, int nx, int ny, int nz, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr || scheme_name == nullptr) {
        std::cerr << "ERROR in cece_fms_scheme_init: null argument" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        for (size_t i = 0; i < scheme_configs.size(); ++i) {
            if (scheme_configs[i].name == name) {
                if (i >= internal_data->active_schemes.size() || internal_data->active_schemes[i] == nullptr) {
                    std::cerr << "ERROR in cece_fms_scheme_init: scheme '" << name << "' exists in config but not in active_schemes" << std::endl;
                    if (rc != nullptr) *rc = -2;
                    return;
                }
                found = true;
        ...
        if (!found) {
            std::cerr << "ERROR in cece_fms_scheme_init: scheme '" << name << "' not found in active_schemes" << std::endl;
            if (rc != nullptr) *rc = -3;
            return;
        }
    } catch (const std::exception& e) {
        std::cerr << "ERROR in cece_fms_scheme_init: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_scheme_init: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}

void cece_fms_scheme_run(void* data_ptr, const char* scheme_name, int scheme_name_len, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr || scheme_name == nullptr) {
        std::cerr << "ERROR in cece_fms_scheme_run: null argument" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        if (scheme_ptr == nullptr) {
            std::cerr << "ERROR in cece_fms_scheme_run: scheme '" << name << "' not found in active_schemes" << std::endl;
            if (rc != nullptr) *rc = -3;
            return;
        }
        ...
        std::cerr << "ERROR in cece_fms_scheme_run: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_scheme_run: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}

void cece_fms_scheme_finalize(void* data_ptr, const char* scheme_name, int scheme_name_len, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr || scheme_name == nullptr) {
        std::cerr << "ERROR in cece_fms_scheme_finalize: null argument" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        if (scheme_ptr == nullptr) {
            std::cerr << "ERROR in cece_fms_scheme_finalize: scheme '" << name << "' not found in active_schemes" << std::endl;
            if (rc != nullptr) *rc = -3;
            return;
        }
        ...
        std::cerr << "ERROR in cece_fms_scheme_finalize: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_scheme_finalize: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}

void cece_fms_set_import_field(void* data_ptr, const char* field_name, int name_len, const double* field_data, int nx, int ny, int nz, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr || field_name == nullptr || field_data == nullptr) {
        std::cerr << "ERROR in cece_fms_set_import_field: null argument" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    if (it != internal_data->import_state.fields.end()) {
        ...
        if (dv.extent(0) != snx || dv.extent(1) != sny || dv.extent(2) != snz) {
            std::cerr << "ERROR in cece_fms_set_import_field: dimension mismatch for '" << name << "'. Expected " << dv.extent(0) << "x"
...
void cece_fms_get_export_field(void* data_ptr, const char* field_name, int name_len, double* field_data, int nx, int ny, int nz, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr || field_name == nullptr || field_data == nullptr) {
        std::cerr << "ERROR in cece_fms_get_export_field: null argument" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    if (it == internal_data->export_state.fields.end()) {
        std::cerr << "ERROR in cece_fms_get_export_field: field '" << name << "' not found in export state" << std::endl;
        if (rc != nullptr) *rc = -3;
        return;
    }
    ...
    if (dv.extent(0) != snx || dv.extent(1) != sny || dv.extent(2) != snz) {
        std::cerr << "ERROR in cece_fms_get_export_field: dimension mismatch for '" << name << "'. DualView is " << dv.extent(0) << "x"
...
void cece_fms_run_stacking(void* data_ptr, int hour, int day_of_week, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr) {
        std::cerr << "ERROR in cece_fms_run_stacking: null data_ptr" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        if (internal_data->config.species_layers.empty()) {
            std::cout << "INFO: cece_fms_run_stacking — no species layers, skipping" << std::endl;
            return;
        }

        if (!internal_data->stacking_engine) {
            std::cerr << "ERROR in cece_fms_run_stacking: stacking_engine is null" << std::endl;
            if (rc != nullptr) *rc = -2;
            return;
        }
        ...
        Kokkos::fence("CECE::FMS::RunStacking");

    } catch (const std::exception& e) {
        std::cerr << "ERROR in cece_fms_run_stacking: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_run_stacking: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}

void cece_fms_core_finalize(void* data_ptr, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr) {
        std::cerr << "ERROR in cece_fms_core_finalize: null data_ptr" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        Kokkos::fence("CECE::FMS::Finalize::PreCleanup");

        std::cout << "INFO: CECE FMS core_finalize — beginning cleanup" << std::endl;
        ...
        Kokkos::fence("CECE::FMS::Finalize::PreDelete");
        ...
        if (should_finalize_kokkos && Kokkos::is_initialized()) {
            Kokkos::finalize();
            std::cout << "INFO: Kokkos finalized (initialized by CECE FMS)" << std::endl;
        } else {
            std::cout << "INFO: Skipping Kokkos finalization (not owned by CECE FMS)" << std::endl;
        }

        std::cout << "INFO: CECE FMS core_finalize completed" << std::endl;

    } catch (const std::exception& e) {
        std::cerr << "ERROR in cece_fms_core_finalize: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_core_finalize: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}

void cece_fms_sync_import_to_device(void* data_ptr, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr) {
        std::cerr << "ERROR in cece_fms_sync_import_to_device: null data_ptr" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        std::cerr << "ERROR in cece_fms_sync_import_to_device: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_sync_import_to_device: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}

void cece_fms_sync_export_to_host(void* data_ptr, int* rc) {
    if (rc != nullptr) *rc = 0;

    if (data_ptr == nullptr) {
        std::cerr << "ERROR in cece_fms_sync_export_to_host: null data_ptr" << std::endl;
        if (rc != nullptr) *rc = -1;
        return;
    }
    ...
    try {
        ...
        Kokkos::fence("CECE::FMS::SyncExportToHost");

    } catch (const std::exception& e) {
        std::cerr << "ERROR in cece_fms_sync_export_to_host: " << e.what() << std::endl;
        if (rc != nullptr) *rc = -1;
    } catch (...) {
        std::cerr << "ERROR in cece_fms_sync_export_to_host: unknown exception" << std::endl;
        if (rc != nullptr) *rc = -1;
    }
}
